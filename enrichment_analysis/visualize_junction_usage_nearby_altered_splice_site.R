...
odds_ratio_concensus_boxplot_across_tissues <- function(aa, output_file) {
...
odds_ratio_ppt_boxplot_across_tissues <- function(aa, output_file) {
	options(bitmapType = 'cairo', device = 'pdf')
...
output_file <- paste0(visualize_jxn_usage_nearby_altered_ss_enrichment_dir, "concensus_jxn_usage_enrichment_across_tissues.pdf")
odds_ratio_concensus_boxplot_across_tissues(enrichment_data, output_file)

# Load in data
input_file <- paste0(jxn_usage_nearby_altered_ss_enrichment_dir, "tissue_by_tissue_outliers_with_rv_in_ppt_sites_outlier_individuals_1e-06_inlier_individuals_0.5_with_read_counts.txt")
enrichment_data <- read.table(input_file, header=TRUE)

# Visualize enrichment distribution (data aggregrated) tissues
output_file <- paste0(visualize_jxn_usage_nearby_altered_ss_enrichment_dir, "ppt_jxn_usage_enrichment_across_tissues.pdf")
odds_ratio_ppt_boxplot_across_tissues(enrichment_data, output_file)