...
def fill_in_tissues_te_outlier_calls(outlier_file_name, gene_individual_pairs, tissue_num):
...
te_outlier_dir = sys.argv[3]  # Directory containing te outlier calls in each tissue
...
output_file = unsupervised_learning_input_dir + 'total_expression_tbt_outliers_' + str(pvalue) + '_genes_intersection_between_te_ase_splicing_features_filter_N2_pairs.txt'
...
for tissue_num, tissue_name in enumerate(tissues):
	print(tissue_num)
	outlier_file_name = te_outlier_dir + tissue_name + '.globalOutliers.v8ciseqtls.removed.outlierGenes.zscores.txt'
	gene_individual_pairs = fill_in_tissues_te_outlier_calls(outlier_file_name, gene_individual_pairs, tissue_num)
