...
for (cluster_num in 1:num_clusters) {
  ...
  outlier_string = as.character(clusters_to_plot[cluster_num,4])
  num_outliers <- length(strsplit(outlier_string,split=",")[[1]])
	membership_vector <- factor(c(rep("outlier",num_outliers), rep("inlier", dim(counts)[1] -num_outliers)))
...
#make_differential_splicing_plot(counts, x=membership_vector,main_title=paste0(cluster_id, " / ",dist_to_ss), snp_pos=var_pos,exons_table=exons_table, output_file=output_file)
