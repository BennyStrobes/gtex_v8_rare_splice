...
def run_step_5_add_watershed_info(step_4_output_file, watershed_posterior_file, step_5_output_file):
	...
	for line in f:
		...
		if float(watershed_info[10]) > .7:
			t.write(variant_id + '\t' + '\t'.join(data[1:]))
			t.write('\t' + watershed_info[4] + '\t' + watershed_info[10] + '\t' + watershed_info[12] + '\t' + watershed_info[11] + '\n')
	t.close()
	f.close()

def run_step_6_check_to_see_if_variant_has_low_watershed_in_any_individuals(step_6_output_file, watershed_posterior_file, step_7_output_file):
	variants = {}
	head_count = 0
	variants = {}
...
def make_clusters_to_plot_file(organized_output_file, cluster_to_ss_mapping, cluster_to_strand_mapping, annotated_splice_sites, whole_blood_outlier_file, clusters_to_plot_file):
	...
	for line in f:
		...
		strand = cluster_to_strand_mapping[cluster_id]
		var_pos = data[4]
		variant_location = int(data[0].split('_')[1])
		if strand == '+':
			if var_pos.startswith('D'):
				# donor ss
				shift = int(var_pos.split('-')[1])
				ss_loc = variant_location + shift - 1
				splice_site = data[0].split('_')[0] + ':' + str(ss_loc)
			else:
				# acceptor ss
				shift = int(var_pos.split('+')[1])
				ss_loc = variant_location - shift + 1
...
def run_step_7_check_for_confounding_variants(step_5_output_file, watershed_posterior_file, step_6_output_file):
	dicti = {}
	variant = {}
	head_count = 0
	...
	for line in f:
		...
		if head_count == 0:
			head_count = head_count + 1
			continue
		gene_indi = data[1] + ':' + data[8]
		dicti[gene_indi] = 0.0
		variant[gene_indi] = data[0]
	f.close()
	f = gzip.open(watershed_posterior_file)
	head_count = 0
	for line in f:
...
outlier_variant_position_file = sys.argv[6]
...
step_1_output_file = processed_data_output_dir + 'test2_step_1_filters.txt'
#run_step_1_filters(outlier_variant_position_file, cluster_to_strand_mapping, step_1_output_file)


step_2_output_file = processed_data_output_dir + 'test2_step_2_filters.txt'
#run_step_2_filters(step_1_output_file, whole_blood_outlier_file, step_2_output_file, 'whole_blood_cluster_splicing_pvalue')
step_3_output_file = processed_data_output_dir + 'test2_step_3_filters.txt'
...
step_4_output_file = processed_data_output_dir + 'test2_step_4_filters.txt'
#run_step_4_add_cluster_info(step_3_output_file, cluster_info_file, step_4_output_file)

# Add Watershed INFO
step_5_output_file = processed_data_output_dir + 'test2_step_5_filters.txt'
#run_step_5_add_watershed_info(step_4_output_file, watershed_posterior_file, step_5_output_file)

step_6_output_file = processed_data_output_dir + 'test2_step_6_filters.txt'
#run_step_6_check_to_see_if_variant_has_low_watershed_in_any_individuals(step_5_output_file, watershed_posterior_file, step_6_output_file)

step_7_output_file = processed_data_output_dir + 'test2_step_7_filters.txt'
#run_step_7_check_for_confounding_variants(step_6_output_file, watershed_posterior_file, step_7_output_file)


organized_output_file = processed_data_output_dir + 'test_variants_near_splice_sites.txt'
organized_with_junctions_output_file = processed_data_output_dir + 'test_variants_neaer_splice_sites_with_junc_positions.txt'
