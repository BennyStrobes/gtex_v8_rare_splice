...
def run_dm_outlier_analysis(X_train, X_test, cov_mat_train, cov_mat_test, DM_GLM, num_reads, model_version):
	#########################################################
	...
	if model_version == 'no_prior_multiple_initializations':
		...
		alpha = dirichlet_multinomial_fit(X_train, DM_GLM)
	...
	num_samples, num_jxns = X_test.shape
	# Loop through samples
	for sample_num in range(num_samples):
		mahalanobis_distances.append(compute_mahalanobis_distance(X_test[sample_num,:], alpha))
